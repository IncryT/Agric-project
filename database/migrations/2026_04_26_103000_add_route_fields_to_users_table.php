<?php
use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::table('users', function (Blueprint $table) {
            $table->string('route_name')->nullable()->after('district');
            $table->string('destination')->nullable()->after('route_name');
            $table->decimal('destination_lat', 10, 8)->nullable()->after('destination');
            $table->decimal('destination_lng', 11, 8)->nullable()->after('destination_lat');
            $table->decimal('distance_to_destination', 12, 2)->nullable()->after('destination_lng');
        });
    }

    public function down(): void
    {
        Schema::table('users', function (Blueprint $table) {
            $table->dropColumn(['route_name', 'destination', 'destination_lat', 'destination_lng', 'distance_to_destination']);
        });
    }
};
