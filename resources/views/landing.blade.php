...
<head>
    ...
    <style>
        ...
        .glow { box-shadow: 0 0 60px rgba(52,211,153,0.3); }
    </style>
</head>
<body class="min-h-screen bg-[#020617] text-slate-100 antialiased selection:bg-emerald-500/30">
    ...
    <div class="sticky top-0 z-50 glass border-b border-white/10 backdrop-blur-2xl">
        <div class="mx-auto max-w-7xl px-6 py-4 flex flex-wrap items-center justify-between gap-4">
            ...
            <nav class="hidden md:flex items-center gap-8 text-sm font-medium text-slate-300"></nav>
            <div class="flex items-center gap-3">
                <a href="{{ route('login') }}" class="rounded-2xl glass border border-emerald-400/30 text-emerald-300 px-5 py-2.5 text-sm font-semibold hover:bg-emerald-500/10 transition">Sign In</a>
                <a href="{{ route('register') }}" class="rounded-2xl bg-gradient-to-r from-emerald-400 to-cyan-400 px-5 py-2.5 text-sm font-bold text-slate-950 shadow-2xl shadow-cyan-500/20 hover:from-emerald-300 hover:to-cyan-300 transition glow">Get Started</a>
            </div>
        </div>
    </div>
    ...
    <section class="relative py-6 overflow-hidden">
        ...
        <div class="relative">
            <div class="flex items-center gap-2 mb-4 ml-6">
                <span class="flex h-2 w-2"><span class="animate-ping absolute inline-flex h-2 w-2 rounded-full bg-emerald-400 opacity-75"></span><span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-400"></span></span>
            ...
            <div class="overflow-hidden bg-slate-900/60 border-y border-white/5">
                <div class="flex min-w-full gap-6 py-3" style="animation: marquee 25s linear infinite;">
        ...
        <div class="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center mb-20">
            <div class="space-y-8">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <a href="{{ route('register') }}" class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-emerald-400 via-cyan-400 to-violet-400 px-8 py-4 text-lg font-bold text-slate-950 shadow-2xl shadow-emerald-500/30 hover:scale-[1.02] hover:shadow-emerald-500/50 transition-all glow">
                        Get Started
                        <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                    </a>
                    <a href="{{ route('login') }}" class="inline-flex items-center justify-center rounded-2xl glass border border-emerald-400/30 text-emerald-300 px-8 py-4 text-lg font-semibold hover:bg-emerald-500/10 transition-all">
                        Sign In
                    </a>
                </div>
            </div>
    ...
    <footer class="border-t border-white/10 py-12">
        <div class="mx-auto max-w-7xl px-6 flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex items-center gap-6 text-sm text-slate-400">
                <a href="{{ route('login') }}" class="hover:text-emerald-400 transition-colors">Admin Portal</a>
